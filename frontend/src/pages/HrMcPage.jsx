...
export function HrMcPage() {
  ...
  const createMutation = useMutation({ mutationFn: (body) => apiFetch('/api/hr-mc', { method: 'POST', body }), onSuccess: () => { invalidate(); setShowForm(false); setForm({ ...EMPTY_FORM }); } });
  const updateMutation = useMutation({ mutationFn: ({ id, body }) => apiFetch(`/api/hr-mc/${id}`, { method: 'PUT', body }), onSuccess: () => { invalidate(); setShowForm(false); setEditingId(null); setForm({ ...EMPTY_FORM }); } });
  const deleteMutation = useMutation({ mutationFn: (id) => apiFetch(`/api/hr-mc/${id}`, { method: 'DELETE' }), onSuccess: () => { invalidate(); setDeleteId(null); } });

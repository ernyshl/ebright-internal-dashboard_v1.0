...
// Set this to the Google Sheet that holds HR event confirmation tabs.
// Per-venue GIDs are entered in the "+ Add Venue" form. Until HR provides
// the sheet ID, the "Confirmed" column will show 0 — that's expected.
const SHEET_BASE_ID = '';
...
async function fetchConfirmedCount(gids) {
  if (!SHEET_BASE_ID || !gids || gids.length === 0) return 0;
  let total = 0;
...
export function HrEventDashboardPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['hrEventVenues'],
    queryFn: () => apiFetch('/api/hr-event/venues'),
    staleTime: 2 * 60 * 1000,
  });

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['hrEventVenues'] });
  const createMutation = useMutation({ mutationFn: (body: any) => apiFetch('/api/hr-event/venues', { method: 'POST', body }), onSuccess: () => { invalidate(); setShowForm(false); setForm({ ...EMPTY_VENUE }); } });
  const updateMutation = useMutation({ mutationFn: ({ id, body }: { id: any; body: any }) => apiFetch(`/api/hr-event/venues/${id}`, { method: 'PUT', body }), onSuccess: () => { invalidate(); setShowForm(false); setEditingId(null); setForm({ ...EMPTY_VENUE }); } });
  const deleteMutation = useMutation({ mutationFn: (id: any) => apiFetch(`/api/hr-event/venues/${id}`, { method: 'DELETE' }), onSuccess: () => invalidate() });
    ...
    <div className="dashboardPage">
      <div className="dashboardHeader">
        ...
        <div style={{ marginTop: 16 }}>
          <h1 className="pageHeaderTitle">HR Event Dashboard</h1>
          <p className="headerSubtitle">HR Event Confirmation Tracker</p>
        </div>

...
export function HrMcPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['hrMcList', search, posFilter, dept, dateFrom, dateTo, page],
    queryFn: () => apiFetch(`/api/hr-mc?${params}`),
    staleTime: 2 * 60 * 1000,
    placeholderData: keepPreviousData,
  });

  const invalidate = () => { queryClient.invalidateQueries({ queryKey: ['hrMcList'] }); queryClient.invalidateQueries({ queryKey: ['hrMcDashboard'] }); };
  const createMutation = useMutation({ mutationFn: (body: any) => apiFetch('/api/hr-mc', { method: 'POST', body }), onSuccess: () => { invalidate(); setShowForm(false); setForm({ ...EMPTY_FORM }); } });
  const updateMutation = useMutation({ mutationFn: ({ id, body }: { id: any; body: any }) => apiFetch(`/api/hr-mc/${id}`, { method: 'PUT', body }), onSuccess: () => { invalidate(); setShowForm(false); setEditingId(null); setForm({ ...EMPTY_FORM }); } });
  const deleteMutation = useMutation({ mutationFn: (id: any) => apiFetch(`/api/hr-mc/${id}`, { method: 'DELETE' }), onSuccess: () => { invalidate(); setDeleteId(null); } });
  ...
  const handleEdit = (r) => { setEditingId(r.id); setForm({ name: r.name || '', position: r.position || '', department_branch: r.department_branch || '', mc_date: r.mc_date ? r.mc_date.split('T')[0] : '', reason: r.reason || '' }); setShowForm(true); };
  ...
  return (
    <div className="dashboardPage">
      <div className="dashboardHeader">
        ...
        <div style={{ marginTop: 16 }}><h1 className="pageHeaderTitle">MC (Medical Certificate)</h1><p className="headerSubtitle">{total} records</p></div>
        <button className="btn btnPrimary btnSmall" onClick={handleAdd} style={{ marginLeft: 'auto' }}>+ Add MC</button>
      </div>

      {showForm && (
        <div className="card" style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <h3 style={{ margin: 0 }}>{editingId ? 'Edit MC' : 'Add New MC'}</h3>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            ...
            <label className="field"><div className="label">MC Date</div><input className="input" type="date" value={form.mc_date} onChange={e => setForm({ ...form, mc_date: e.target.value })} required /></label>
            <label className="field"><div className="label">Reason</div><input className="input" value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} placeholder="e.g. Flu, Fever" /></label>
            <div style={{ display: 'flex', alignItems: 'flex-end' }}><button className="btn btnPrimary" type="submit" disabled={isSaving}>{isSaving ? 'Saving...' : (editingId ? 'Save' : 'Add MC')}</button></div>
      ...
      <div className="brRankFilters" style={{ marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div className="brRankFilterGroup" style={{ flex: 1, minWidth: 180 }}><label className="brRankLabel">Search</label><input className="filterInput" placeholder="Name / Reason" value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%' }} /></div>
      ...
      {isLoading ? (<div className="card" style={{ textAlign: 'center', padding: 40 }}><div className="loadingDots"><span /><span /><span /></div></div>
        ...
        <div className="card" style={{ overflowX: 'auto', padding: 0 }}>
          <table className="dataTable">
            <thead><tr><th>#</th><th>Name</th><th>Position</th><th>Dept / Branch</th><th>MC Date</th><th>Reason</th><th>Actions</th></tr></thead>
            <tbody>
              {records.length === 0 ? (<tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--muted)', padding: 32 }}>No records</td></tr>
                ...
                <tr key={r.id}>
                  ...
                  <td style={{ whiteSpace: 'nowrap' }}>{fmtDate(r.mc_date)}</td><td>{r.reason || '—'}</td>
      ...
      {deleteId && (
        <div className="modalOverlay" onClick={() => setDeleteId(null)}><div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}><div className="modalHeader"><div className="modalTitle">Delete MC Record</div><button className="modalClose" onClick={() => setDeleteId(null)}>✕</button></div><div className="modalBody"><p>Delete this MC record?</p><div style={{ display: 'flex', gap: 10, marginTop: 16 }}><button className="btn btnDanger" onClick={() => deleteMutation.mutate(deleteId)} disabled={deleteMutation.isPending}>{deleteMutation.isPending ? 'Deleting...' : 'Delete'}</button><button className="btn btnGhost" onClick={() => setDeleteId(null)}>Cancel</button></div></div></div></div>
      )}
    </div>
  );
}

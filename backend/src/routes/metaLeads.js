...
// Normalise a Meta form branch field value → LOCATION_MAPPING key
// Meta sends values like "kota_damansara_(the_strand)", "subang_taipan", "online", etc.
function resolveMetaBranch(rawBranch) {
  if (!rawBranch) return null;
  const normalized = rawBranch
    .toLowerCase()
    .trim()
    .replace(/\s*\([^)]*\)\s*/g, '')  // strip "(the strand)" etc.
...
async function pushToGHL(fullName, email, phone, locationKey) {
  ...
  if (!locationConfig) {
    console.warn(`[Meta → GHL] No location config for key: ${locationKey}`);
    return;
  }
  ...
  if (!contactId) {
    console.error('[Meta → GHL] Contact creation failed:', JSON.stringify(contactRes.body));
    return;
  }
  ...
  if (!pipeline) {
    console.warn(`[Meta → GHL] No pipeline found for ${locationKey}`);
    return;
  }
  ...
  console.log(`[Meta → GHL] Contact + opportunity created for ${email} in ${locationKey}`);
}
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    if (env.META_GHL_WEBHOOK_KEY && payload.webhook_key !== env.META_GHL_WEBHOOK_KEY) {
    ...
    const locationKey = resolveMetaBranch(rawBranch);
    if (!locationKey) {
      console.warn(`[Meta → GHL] Could not resolve branch: "${rawBranch}"`);
    ...
    pushToGHL(fullName, email, phone, locationKey)
      .catch(err => console.error('[Meta → GHL] push failed:', err.message));

    return res.status(200).json({ status: 'ok', ghl: 'queued', branch: locationKey });
  } catch (err) {
    console.error('[Meta GHL webhook]', err.message);
    return res.status(200).json({ status: 'error', message: err.message });
  }
});

module.exports = { metaLeadsRouter: router };

-- 017_master_leads_base_add_children_count.sql
--
-- 1. Add children_count (integer, nullable) to master_leads_base.
--    Meta and TikTok rows remain NULL. Wix rows carry the value from
--    raw_wix_leads.children_count.
-- 2. Backfill existing Wix rows.
-- 3. Replace refresh_master_leads_base() to include children_count on new inserts.
--
...
SET children_count = rw.children_count
...
WHERE mlb.source NOT IN ('Meta', 'TikTok')
  ...
  AND mlb.children_count IS NULL;
...
BEGIN
  ...
  WITH src AS (
    ...
    INSERT INTO master_leads_base (source, full_name, email, phone, branch, submission_date, children_count, children_details)
    SELECT lead_source, full_name, email, phone, branch, submission_date, children_count, children_details
    FROM src
    WHERE email IS NOT NULL AND email <> ''
